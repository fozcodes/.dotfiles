...
import type { Insight, ProjectStats, ProviderStats, TotalStats } from "./data.ts";
...
/** One row per project, then a TOTAL row. */
export function buildProjectTableCsv(projects: ReadonlyMap<string, ProjectStats>, totals: TotalStats): string {
	const lines = [
		csvLine([
			"project",
			"sessions",
	...
	const sorted = Array.from(projects.entries()).sort((a, b) => b[1].cost - a[1].cost);
