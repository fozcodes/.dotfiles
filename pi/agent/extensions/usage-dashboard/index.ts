...
import type { BaseStats, ProjectStats, ProviderStats, TabName, TotalStats, UsageData } from "./data";
import {
	buildGraphModel,
	renderChart,
	GROUP_LABELS,
...
type TableGroupBy = "provider" | "project";
...
class UsageComponent {
	...
	private tableGroupBy: TableGroupBy = "provider";
	...
	private clampTableSelection(): void {
		const count = this.tableGroupBy === "project" ? this.visibleProjectTable().projects.size : this.visibleTable().providers.size;
	...
	private visibleProjectTable(): { projects: Map<string, ProjectStats>; totals: TotalStats } {
		const stats = this.data[this.activeTab];
		const q = this.tableFilter.trim().toLowerCase();
		const projects = new Map<string, ProjectStats>();
		const ordered = Array.from(stats.projects.entries()).sort((a, b) => b[1].cost - a[1].cost);
		for (const [name, project] of ordered) {
			if (this.tableHidden.has(name)) continue;
			if (!q || name.toLowerCase().includes(q)) projects.set(name, project);
		}
		if (!q && this.tableHidden.size === 0) return { projects, totals: stats.totals };
		...
		for (const project of projects.values()) {
			totals.messages += project.messages;
			totals.cost += project.cost;
			totals.tokens.total += project.tokens.total;
			totals.tokens.input += project.tokens.input;
			totals.tokens.output += project.tokens.output;
			totals.tokens.cacheRead += project.tokens.cacheRead;
			totals.tokens.cacheWrite += project.tokens.cacheWrite;
			for (const session of project.sessions) sessions.add(session);
		}
		totals.sessions = sessions.size;
	...
	handleInput(data: string): void {
		...
		if (this.viewMode === "table" && matchesKey(data, "g")) {
			this.tableGroupBy = this.tableGroupBy === "provider" ? "project" : "provider";
		...
		if (matchesKey(data, "tab") || matchesKey(data, "right")) {
			...
			const visible = this.tableGroupBy === "project" ? Array.from(this.visibleProjectTable().projects.keys()) : Array.from(this.visibleTable().providers.keys());
			...
			const provider = this.tableGroupBy === "provider" ? Array.from(this.visibleTable().providers.keys())[this.selectedIndex] : undefined;
			if (provider) {
	...
	private exportCurrentView(): void {
		...
		} else if (this.tableGroupBy === "project") {
			const visible = this.visibleProjectTable();
			const sliced = this.tableFilter.trim() !== "" || this.tableHidden.size > 0;
			name = exportFileName("table", this.activeTab, `project${sliced ? "-filtered" : ""}`, "csv", now);
			content = buildProjectTableCsv(visible.projects, visible.totals);
		} else {
	...
	private renderTabs(width: number, layout: TableLayout): string[] {
		...
		if (this.viewMode === "table") {
			infoLines.push(th.fg("dim", `Grouped ${this.tableGroupBy === "project" ? "by project" : "by provider / model"} · [g] switch`));
	...
	private renderHeader(layout: TableLayout): string[] {
		...
		const nameLabel = this.tableGroupBy === "project" ? "Project" : "Provider / Model";
		let headerLine = fitCell(nameLabel, layout.nameWidth);
	...
	private renderRows(layout: TableLayout): string[] {
		...
		if (this.tableGroupBy === "project") {
			const projects = Array.from(this.visibleProjectTable().projects.entries());
			if (projects.length === 0) {
				lines.push(th.fg("dim", this.data[this.activeTab].projects.size === 0 ? "  No usage data for this period" : "  Nothing matches the current filter — [a] resets"));
				return lines;
			}
			for (let i = 0; i < projects.length; i++) {
				const [projectName, projectStats] = projects[i]!;
				const isSelected = i === this.selectedIndex;
				const prefix = isSelected ? th.fg("accent", "▸ ") : th.fg("dim", "  ");
				lines.push(this.renderDataRow(projectName, projectStats, layout, { selected: isSelected, prefix }));
			}
			return lines;
		}
	...
	private renderTotals(layout: TableLayout): string[] {
		const th = this.theme;
		const { totals } = this.tableGroupBy === "project" ? this.visibleProjectTable() : this.visibleTable();
	...
	private renderHelp(width: number): string[] {
		...
		const variants =
			this.viewMode === "graph"
				...
				: [
						"[Tab/←→] period  [↑↓] select  [g] group  [Enter] expand  [/] filter  [x] hide  [a] all  [e] export  [v] view  [q] close",
						"[Tab] period  [↑↓] select  [g] group  [Enter] expand  [/] filter  [x] hide  [e] export  [v] view  [q] close",
						"[↑↓] select  [g] group  [Enter] expand  [/] filter  [x] hide  [v] view  [q] close",
						"[↑↓] select  [/] [x] [v] [q]",
						"[↑↓] select  [q] close",
						"[q] close",
				  ];
