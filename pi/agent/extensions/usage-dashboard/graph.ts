...
export type GraphGroupBy = "project" | "provider" | "model" | "thinking" | "total";

export const METRIC_ORDER: GraphMetric[] = ["cost", "tokens", "messages", "reasoning"];
export const GROUP_ORDER: GraphGroupBy[] = ["project", "provider", "model", "thinking", "total"];
...
function groupKeyOf(key: HourlyKey, groupBy: GraphGroupBy): string {
	...
	const { project, provider, model, thinkingLevel } = splitHourlyKey(key);
	if (groupBy === "project") return project === "" ? "unknown" : project;
...
export function buildGraphModel(
	...
	for (const [hour, bucket] of hourly) {
		...
		for (const [key, cell] of bucket) {
			...
			if (value === 0) continue;
			totalPoints[idx] = totalPoints[idx]! + value;
			totalSum += value;
	...
	if (merged.length > 0) {
		...
		for (const [groupKey, groupTotal] of merged) {
			...
			for (let i = 0; i < bucketCount; i++) points[i] = points[i]! + groupPoints[i]!;
			total += groupTotal;
		}
		series.push({
