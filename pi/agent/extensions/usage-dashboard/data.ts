...
/**
 * One (project, provider, model, thinkingLevel) cell inside an hourly bucket.
...
/** Composite key: `${project}\u0000${provider}\u0000${model}\u0000${thinkingLevel}` */
export type HourlyKey = string;

export const HOURLY_KEY_SEP = "\u0000";

export function makeHourlyKey(provider: string, model: string, thinkingLevel: string, project = ""): HourlyKey {
	return project + HOURLY_KEY_SEP + provider + HOURLY_KEY_SEP + model + HOURLY_KEY_SEP + thinkingLevel;
}

export function splitHourlyKey(key: HourlyKey): { project: string; provider: string; model: string; thinkingLevel: string } {
	const parts = key.split(HOURLY_KEY_SEP);
	if (parts.length >= 4) {
		const [project = "", provider = "", model = "", thinkingLevel = ""] = parts;
		return { project, provider, model, thinkingLevel };
	}
	const [provider = "", model = "", thinkingLevel = ""] = parts;
	return { project: "", provider, model, thinkingLevel };
}
...
function addToHourlyBuckets(hourly: Map<number, Map<HourlyKey, HourlyCell>>, msg: SessionMessage, project: string): void {
	...
	const key = makeHourlyKey(msg.provider, msg.model, msg.thinkingLevel, project);
...
function addMessagesToUsageData(
	...
	for (let mi = 0; mi < messages.length; mi++) {
		...
		addToHourlyBuckets(data.hourly, msg, project);
		...
		for (const period of periods) {
			...
			let projectStats = stats.projects.get(project);
			if (!projectStats) {
				projectStats = emptyProjectStats();
				stats.projects.set(project, projectStats);
			}
			projectStats.sessions.add(sessionId);
			accumulateStats(projectStats, msg.cost, tokens, isAssistant);
		...
		for (const m of rawMsgs) {
			...
			const hash =
				m.source === "auxiliary" && m.sourceId
					? `auxiliary:${state.parsed.sessionId}:${m.sourceId}:${m.timestamp}:${tokenFingerprint}`
					: `${m.source}:${state.parsed.sessionId}:${m.timestamp}:${tokenFingerprint}`;
